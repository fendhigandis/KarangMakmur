...
async function prosesLogin(e) {
    ...
    // Konversi username menjadi email untuk Firebase Auth
    let emailLogin = u + "@bumdes.com";

    Swal.fire({ title: 'Otentikasi...', text: 'Memeriksa kredensial aman di Cloud', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); }});

    try {
        // PERBAIKAN: Menggunakan Firebase Auth
        await auth.signInWithEmailAndPassword(emailLogin, p);
        
        // Atur Role Berdasarkan Username (Bisa disesuaikan dengan Firestore Claims nanti)
        currentUser = { 
            name: u.toUpperCase(), 
            role: u === 'admin' ? 'Super Admin' : 'Pengelola Unit', 
            unit: u === 'admin' ? 'Semua' : u 
        };
        ...
        Swal.fire({ icon: 'error', title: 'Akses Ditolak!', text: 'Username atau Password salah. Jika Anda belum mendaftarkan user ini di Firebase Auth, silakan daftarkan melalui tab Hak Akses.' });
    }
}

function prosesLogout() {
    if (currentUser) catatLog('Autentikasi Logout', `User '${currentUser.name}' telah keluar dari sistem.`);
    auth.signOut().then(() => {
