...
function injectModals() {
    ...
    const modalHTML = `
        ...
        <!-- PENAMBAHAN: Pratinjau Cetak PDF -- hasil PDF (Laporan/LPJ) ditampilkan dulu di sini
             sebelum diunduh atau dicetak, sehingga pengguna bisa mengecek hasilnya terlebih dahulu. -->
        <div class="modal fade" id="modalPreviewCetak" tabindex="-1"><div class="modal-dialog modal-dialog-centered modal-xl"><div class="modal-content border-0 shadow-lg rounded-4"><div class="modal-header border-bottom"><h6 class="modal-title fw-bold" id="judul-modal-preview-cetak"><i class="fas fa-file-pdf me-1"></i> Pratinjau Cetak</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-0"><iframe id="iframe-preview-cetak" class="iframe-preview-cetak" title="Pratinjau PDF"></iframe></div><div class="modal-footer border-top"><button type="button" class="btn btn-outline-secondary fw-bold" data-bs-dismiss="modal"><i class="fas fa-times me-1"></i> Tutup</button><button type="button" class="btn btn-outline-primary fw-bold" id="btn-cetak-preview-cetak"><i class="fas fa-print me-1"></i> Cetak</button><button type="button" class="btn text-white fw-bold" id="btn-unduh-preview-cetak" style="background: var(--accent-teal);"><i class="fas fa-download me-1"></i> Unduh PDF</button></div></div></div></div>
...
// PENAMBAHAN: menampilkan hasil PDF (Laporan/LPJ) dalam jendela pratinjau terlebih dahulu, agar
// pengguna bisa mengecek isinya sebelum benar-benar mengunduh atau mengirimnya ke printer. Dipakai
// bersama oleh cetakLaporanAktif() dan cetakLPJPDF() supaya perilakunya konsisten di semua laporan.
function tampilkanPreviewPDF(doc, namaFile, judulPreview) {
    let modalEl = document.getElementById('modalPreviewCetak');
    let iframe = document.getElementById('iframe-preview-cetak');
    let btnUnduh = document.getElementById('btn-unduh-preview-cetak');
    let btnCetak = document.getElementById('btn-cetak-preview-cetak');
    if (!modalEl || !iframe || !btnUnduh || !btnCetak) {
        // Jaga-jaga (mis. injectModals belum sempat jalan): tetap unduh langsung agar tidak gagal total.
        doc.save(namaFile);
        return;
    }

    let judulEl = document.getElementById('judul-modal-preview-cetak');
    if (judulEl) judulEl.innerHTML = `<i class="fas fa-file-pdf me-1"></i> Pratinjau Cetak${judulPreview ? ' - ' + judulPreview : ''}`;
    ...
    btnCetak.onclick = () => {
        try {
            ...
            // Jaga-jaga jika browser memblokir print dari dalam iframe: buka di tab baru sebagai cadangan.
        ...
        try {
            ...
            tabelDitemukan.forEach(t => {
                doc.autoTable({
                    ...
                    // PERBAIKAN: sebelumnya margin.top tidak diatur (memakai bawaan AutoTable ~20mm),
                    // padahal kop surat (nama BUMDes + alamat + garis + judul laporan + tanggal cetak)
                    // memakan tempat sampai y=42mm. Akibatnya setiap kali tabel berlanjut ke halaman
                    // baru, baris judul tabel ("NR AKUN | NAMA AKUN" dst.) sudah digambar duluan di
                    // ~20mm, lalu kop surat digambar MENIMPA di atasnya lewat didDrawPage -> tulisan
                    // jadi menumpuk & tidak terbaca. Dengan margin.top = 42 (persis tinggi kop surat),
                    // baris tabel di halaman manapun baru mulai digambar SETELAH kop surat selesai.
                    margin: { top: 42, left: 12, right: 12 },
            ...
            Swal.close();
            tampilkanPreviewPDF(doc, namaFile, judul);
            catatLog('Cetak Laporan', `Membuka pratinjau PDF untuk laporan: ${judul}.`);
        } catch (e) {
...
function cetakLPJPDF() {
    ...
    setTimeout(() => {
        try {
            ...
            Swal.close();
            tampilkanPreviewPDF(doc, namaFile, `LPJ ${tahunSekarang}`);
            catatLog('Cetak LPJ', `Membuka pratinjau PDF Laporan Pertanggungjawaban tahun ${tahunSekarang}.`);
        } catch (e) {
