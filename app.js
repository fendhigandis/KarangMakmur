...
function cetakLaporanAktif() {
    ...
    wrapper.style.cssText = 'position:fixed; left:-99999px; top:0; width:1100px; background:#fff; padding:24px;';

    let kop = document.querySelector('.kop-cetak-resmi');
    if (kop) wrapper.appendChild(kop.cloneNode(true));
    ...
    const opt = { margin: 8, filename: namaFile, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }, pagebreak: { mode: ['css', 'legacy'] } };
