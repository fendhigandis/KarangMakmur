...
function renderJurnalUmum(dataManual) {
    ...
    let dataTampilUrut = [...dataTampil].sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal));
...
function backupDatabaseExcel() {
    ...
    try {
        ...
        let sheetAset = asetTetapData.map(a => ({
            nama: a.nama || '', unit: a.unit || '', tahun: a.tahun || '', harga: a.harga || 0,
        ...
        if (workbook.SheetNames.includes('Aset Tetap')) {
            ...
            for (let r of rows) {
                ...
                await db.collection("asets").add({
                    ...
                    susutPerTahun: parseFloat(r.susutPerTahun) || 0
                });
                totalDipulihkan++;
            }
        }
