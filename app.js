...
// =========================================================================
// PENAMBAHAN: CETAK UNIVERSAL -> SELALU MENGHASILKAN BERKAS PDF ASLI (BUKAN PRINTSCREEN/DIALOG PRINT BROWSER)
// =========================================================================
// Menggantikan window.print() (yang hasilnya seperti "printscreen" & tidak konsisten antar perangkat/printer)
// dengan pembuatan file PDF sungguhan lewat html2pdf untuk laporan/tab APAPUN yang sedang aktif dilihat
// pengguna, sehingga tombol "Cetak" di seluruh aplikasi selalu menghasilkan sebuah berkas yang bisa diunduh.
// =========================================================================
// PERBAIKAN KRITIS: hasil "Unduh PDF" universal SELALU KOSONG
// =========================================================================
// Penyebab #1: class ".kop-cetak-resmi" secara BAWAAN "display:none" di layar (hanya tampil lewat
// @media print milik window.print(), yang TIDAK pernah diproses oleh html2canvas) -> kop surat hasil
// clone selalu tak terlihat/kosong saat dirender ke PDF.
// Penyebab #2: elemen manapun yang memakai class responsif Bootstrap (mis. "d-none d-md-*") akan tetap
// mengikuti lebar layar ASLI perangkat saat dirender -> di HP (<768px) elemen tsb tetap tersembunyi
// walau sudah di-clone. Diperbaiki dengan memaksa windowWidth virtual & melepas class "d-none".
// =========================================================================
function cetakLaporanAktif() {
    ...
    // Tab LPJ sudah memiliki generator PDF multi-halaman (dengan sampul) miliknya sendiri -> arahkan ke sana.
    ...
    // PERBAIKAN: posisi dibuat tetap berada dalam area layar (bukan koordinat ekstrem di luar layar)
    // agar html2canvas selalu berhasil menangkap konten secara utuh di semua browser/perangkat. Wrapper
    // ditumpuk di BAWAH dialog loading (SweetAlert2, z-index 1060) sehingga tetap tidak terlihat pengguna.
    wrapper.style.cssText = 'position:fixed; top:0; left:0; width:100%; max-width:1100px; margin:0 auto; right:0; background:#fff; padding:24px; z-index:1059; overflow:auto; max-height:100vh;';

    let kop = document.querySelector('.kop-cetak-resmi');
    if (kop) {
        let kopClone = kop.cloneNode(true);
        // PERBAIKAN: paksa tampil (menyamai aturan @media print aslinya) karena bawaannya "display:none" di layar
        kopClone.style.setProperty('display', 'flex', 'important');
        kopClone.style.setProperty('flex-direction', 'row', 'important');
        kopClone.style.setProperty('align-items', 'center', 'important');
        kopClone.style.setProperty('justify-content', 'center', 'important');
        wrapper.appendChild(kopClone);
    }

    let judulEl = document.createElement('h5');
    judulEl.style.cssText = 'text-align:center; margin:18px 0; font-weight:800; text-transform:uppercase;';
    judulEl.innerText = judul;
    wrapper.appendChild(judulEl);

    let klon = tabAktif.cloneNode(true);
    klon.style.setProperty('display', 'block', 'important');
    klon.querySelectorAll('.no-print').forEach(el => el.remove());
    // PERBAIKAN: lepas class Bootstrap "d-none" (penyembunyi responsif) agar konten yang bergantung pada
    // lebar layar asli perangkat (mis. di HP) tetap ikut tercetak lengkap di berkas PDF.
    klon.querySelectorAll('.d-none').forEach(el => el.classList.remove('d-none'));
    // Ganti elemen input/select/textarea interaktif menjadi teks statis agar nilainya ikut tercetak di PDF
    klon.querySelectorAll('select, input, textarea').forEach(el => {
        let teks = '';
        if (el.tagName === 'SELECT') teks = el.options[el.selectedIndex] ? el.options[el.selectedIndex].text : '';
        else teks = el.value || '';
    ...
    // PERBAIKAN: windowWidth dipatok 1100 agar html2canvas selalu merender dalam "lebar layar desktop
    // virtual" (bukan lebar layar HP yang sebenarnya), sehingga tabel/laporan tercetak utuh apapun
    // perangkat yang dipakai untuk mengunduh PDF-nya.
    const opt = { margin: 8, filename: namaFile, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, windowWidth: 1100, windowHeight: wrapper.scrollHeight || 1400 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }, pagebreak: { mode: ['css', 'legacy'] } };

    Swal.fire({ title: 'Menyiapkan Berkas PDF...', text: 'Sistem sedang merender laporan menjadi file PDF, mohon tunggu...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
...
// =========================================================================
// PERBAIKAN KRITIS: hasil cetak PDF (LPJ) SELALU KOSONG di layar sempit (HP/tablet <768px)
// =========================================================================
// Penyebab: elemen #lpj-print-area berada di dalam wrapper ".lpj-preview-wrapper" yang memakai
// class Bootstrap "d-none d-md-flex" -> di layar < 768px, wrapper ini benar-benar "display:none",
// sehingga saat html2canvas menangkap #lpj-print-area, hasilnya kosong total (karena elemen leluhurnya
// tidak tampil). Perbaikan berikut memaksa wrapper tampil HANYA sesaat selama proses render PDF
// berlangsung (tidak terlihat pengguna, karena tertutup dialog loading), lalu mengembalikan tampilan
// aslinya persis seperti semula setelah PDF selesai dibuat -- tanpa mengubah desain/CSS aplikasi.
function cetakLPJPDF() {
    ...
    let wrapperEl = element.closest('.lpj-preview-wrapper');

    let styleAsliWrapper = wrapperEl ? wrapperEl.getAttribute('style') : null;

    if (wrapperEl) { wrapperEl.style.setProperty('display', 'flex', 'important'); }

    let namaFile = `LPJ_BUMDes_${(profilBUMDes.nama || 'BUMDes').replace(/\s+/g,'_')}_${new Date().getFullYear()}.pdf`;

    // PERBAIKAN: tambahkan windowWidth agar html2canvas selalu merender dalam "lebar layar desktop virtual"
    // sehingga elemen apapun yang bergantung pada breakpoint responsif (mis. d-md-*) selalu tampil benar,
    // apapun ukuran layar perangkat asli yang dipakai untuk mencetak (HP, tablet, ataupun desktop).
    const opt = { margin: 0, filename: namaFile, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, windowWidth: 1100, windowHeight: element.scrollHeight || 1400 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: ['css', 'legacy'] } };
    ...
    setTimeout(() => {
        ...
        html2pdf().set(opt).from(element).save().then(() => {
            ...
            // PERBAIKAN: kembalikan tampilan wrapper persis seperti semula (agar UI normal tidak berubah)
            if (wrapperEl) { if (styleAsliWrapper === null) wrapperEl.removeAttribute('style'); else wrapperEl.setAttribute('style', styleAsliWrapper); }
